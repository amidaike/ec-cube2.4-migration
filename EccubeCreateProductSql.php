    ...
    public function writeProductClassSql()
    {
        ...
        foreach ($rows as $item) {
            ...
            $productClassValues[] = "($id, $product_id, $product_type_id, $class_category_id1, $class_category_id2, $delivery_date_id,"
                ." $product_code, $stock, $stock_unlimited, $sale_limit, $price01, $price02, $delivery_fee, $creator_id, $create_date, $update_date)";
            $product_stock_id++;
        ...
        if (!empty($productClassValues)) {
            $importProductClassSql = 'INSERT INTO dtb_product_class (product_class_id, product_id, product_type_id, class_category_id1, class_category_id2, delivery_date_id, product_code, stock, stock_unlimited, sale_limit, price01, price02, delivery_fee, creator_id, create_date, update_date) VALUES'.PHP_EOL;
    ...
    public function writeProductTagSql()
    {
        ...
        foreach ($rows as $item) {
            ...
            if ($item['product_flag'] != '00000') {
                ...
                $one = substr($item['product_flag'], 0, 1);
                $two = substr($item['product_flag'], 1, 1);
                $three = substr($item['product_flag'], 2, 1);
                $four = substr($item['product_flag'], 3, 1);
                $five = substr($item['product_flag'], 4, 1);
