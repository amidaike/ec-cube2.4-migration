...
class EccubeCreateOrderSql
{
    ...
    /**
     * dtb_order、dtb_shippingのinsert文を作成
     *
     * @return bool
     */
    public function writeOrderSql()
    {
        ...
        for ($count = 1; $count <= $turn; $count++) {
            ...
            for ($i = $loop; $i < $maxImport * $count; $i++) {
                ...
                $shipping_pref = $item['deliv_pref'] != '' ? $item['deliv_pref'] : 'NULL';
                $delivery_id = $item['deliv_id'] != '' ? $item['deliv_id'] : 'NULL';
                $time_id = $item['deliv_time_id'] != '' ? $item['deliv_time_id'] : 'NULL';
                $fee_id = 'NULL';

                $shipping_name01 = $item['deliv_name01'] != '' ? "'".str_replace("'", "\'", $item['deliv_name01'])."'" : 'NULL';
                $shipping_name02 = $item['deliv_name02'] != '' ? "'".str_replace("'", "\'", $item['deliv_name02'])."'" : 'NULL';
                $shipping_kana01 = $item['deliv_kana01'] != '' ? "'".str_replace("'", "\'", $item['deliv_kana01'])."'" : 'NULL';
                $shipping_kana02 = $item['deliv_kana02'] != '' ? "'".str_replace("'", "\'", $item['deliv_kana02'])."'" : 'NULL';

                $shipping_company_name = 'NULL';

                $shipping_tel01 = $item['deliv_tel01'] != '' ? "'".$item['deliv_tel01']."'" : 'NULL';
                $shipping_tel02 = $item['deliv_tel02'] != '' ? "'".$item['deliv_tel02']."'" : 'NULL';
                $shipping_tel03 = $item['deliv_tel03'] != '' ? "'".$item['deliv_tel03']."'" : 'NULL';
                $shipping_fax01 = $item['deliv_fax01'] != '' ? "'".$item['deliv_fax01']."'" : 'NULL';
                $shipping_fax02 = $item['deliv_fax02'] != '' ? "'".$item['deliv_fax02']."'" : 'NULL';
                $shipping_fax03 = $item['deliv_fax03'] != '' ? "'".$item['deliv_fax03']."'" : 'NULL';

                $shipping_zip01 = $item['deliv_zip01'] != '' ? "'".$item['deliv_zip01']."'" : 'NULL';
                $shipping_zip02 = $item['deliv_zip02'] != '' ? "'".$item['deliv_zip02']."'" : 'NULL';
                ...
                $shipping_addr01 = $item['deliv_addr01'] != '' ? "'".str_replace("'", "\'", $item['deliv_addr01'])."'" : 'NULL';
                $shipping_addr02 = $item['deliv_addr02'] != '' ? "'".str_replace("'", "\'", $item['deliv_addr02'])."'" : 'NULL';

                $shipping_delivery_name = 'NULL';
                $shipping_delivery_time = $item['deliv_time'] != '' ? "'".str_replace("'", "\'", $item['deliv_time'])."'" : 'NULL';
                $shipping_delivery_date = 'NULL';

                $shipping_delivery_fee = $item['deliv_fee'] != '' ? "'".$item['deliv_fee']."'" : 'NULL';
                $shipping_commit_date = $item['commit_date'] != '' ? "'".$item['commit_date']."'" : 'NULL';
                $rank = 'NULL';


                $shippingValues[] = "($shipping_id, $shipping_country_id, $shipping_pref, $id, $delivery_id, $time_id, "
    ...
    {
        $sql = "SELECT detail.*, pcl.product_class_id, c1.name as name1, c2.name as name2"
            ." FROM dtb_order_detail detail "
            ." INNER JOIN dtb_order o ON o.order_id = detail.order_id"
            ." LEFT JOIN dtb_products_class pcl ON detail.product_id = pcl.product_id "
            ." AND (detail.classcategory_id1= pcl.classcategory_id1 or detail.classcategory_id1 = 0)"
            ." AND (detail.classcategory_id2= pcl.classcategory_id2 or detail.classcategory_id2 = 0)"
            ." LEFT JOIN dtb_classcategory cc1 ON detail.classcategory_id1 = cc1.classcategory_id"
            ." LEFT JOIN dtb_classcategory cc2 ON detail.classcategory_id2 = cc2.classcategory_id"
        ...
        for ($count = 1; $count <= $turn; $count++) {
            ...
            for ($i = $loop; $i < $maxImport * $count; $i++) {
                ...
                $shipmentItemValues[] = "($item_id, $order_id, $product_id, $product_class_id, $shipping_id,"
